...
static kRotationMagnitudeThreshold : f32 = 2000f32;
...
pub fn read () {
	unsafe {
		...
		if gmag > kRotationMagnitudeThreshold {
			if !rotating {
				usb_keyboard_press_keycode((bindings::KEY_A  +mindex as u16));
			}
			if rotating && rotation != mindex {
				usb_keyboard_release_keycode((bindings::KEY_A  + rotation as u16));
				usb_keyboard_press_keycode((bindings::KEY_A  + mindex as u16));
			}
...
pub fn post () {
	unsafe {
		...
		for k in 0..keys.len() {
			if !keyCache[k] && keys[k] {
				usb_keyboard_press_keycode((bindings::KEY_A  + 'L' as u16 - 'A' as u16 + k as u16));
			} else if keyCache[k] && !keys[k] {
				usb_keyboard_release_keycode((bindings::KEY_A  + 'L' as u16 - 'A' as u16 + k as u16));
			}
